    ...
    // var chart_z1 = echarts.init(document.getElementById('chart_z1'));
    // var chart_z2 = echarts.init(document.getElementById('chart_z2'));
    ...
    var channel_z = ["APP", "微信", "淘宝"];  // chatZ 的渠道
    ...
    function loadA(start,endt,chan){//开始时间，结束时间，渠道
        ...
        var urlA = "http://shop.daydaycook.com.cn/ecom/appserver/report/orderGmvReport?startDate=" + start + "&endDate=" + endt + "&channelArr=" + chanStrA;
    ...
    function loadB(start,endt,startdb,endtdb,chan){
        ...
        var urlB = "http://shop.daydaycook.com.cn/ecom/appserver/report/orderGmvReport?startDate=" + start + "&endDate=" + endt + "&channelArr=" + chanStrB;
        var urlB_db = "http://shop.daydaycook.com.cn/ecom/appserver/report/orderGmvReport?startDate=" + startdb + "&endDate=" + endtdb + "&channelArr=" + chanStrB;
        var dataAmtB  = [];
        var dataAmtB_db  = [];
        var dayB  = [];
        var dayB_db  = [];
    ...
    function loadC(start,endt,chan){//开始时间，结束时间，渠道
        ...
        var urlC = "http://shop.daydaycook.com.cn/ecom/appserver/report/orderCountReport?startDate=" + start + "&endDate=" + endt + "&channelArr=" + chanStrC;
        var dataCmtC_a  = [];
        var dataCmtC_b  = [];
        var dataCmtC_c  = [];
        var dataCmtC_d  = [];
    ...
    function loadD(start,endt,chan){//开始时间，结束时间，渠道
        ...
        var urlD = "http://shop.daydaycook.com.cn/ecom/appserver/report/orderNumReport?startDate=" + start + "&endDate=" + endt + "&channelArr=" + chanStrD;
        var dataDmtD_a  = [];
        var dataDmtD_b  = [];
        var dataDmtD_c  = [];
        var dayD  = [];
    ...
    function loadE(start,endt,chan,sort){//开始时间，结束时间，渠道
        ...
        var urlE = "http://shop.daydaycook.com.cn/ecom/appserver/report/orderCycleData?startDate=" + start + "&endDate=" + endt + "&channelArr=" + chanStrE;
        var topTenUrl = "http://shop.daydaycook.com.cn/ecom/appserver/report/topProductInfo?startDate=" + start + "&endDate=" + endt + "&channelArr=" + chanStrE + "&sortName=" + sort;
    ...
    function loadF(order){
        var urlD = "http://shop.daydaycook.com.cn/ecom/appserver/report/cycleContrast";
        $.ajax({
            url:urlD,
            type:"GET",
            async:false,
            success:function(obj){
                if(obj.code == "200"){
                    var dataF = obj.data;
                    var lenF = dataF.length;
                    var i = lenF;
                    var strF_app = '';
                    var strF_wechat = '';
                    var strF_taobao = '';
                    for(i;i--;i<0){
                        if(dataF[i].channel == "APP" || dataF[i].channel == "App"){
                            strF_app = "<tr>";
                            strF_app += "<td>" + dataF[i].date + "</td>";
                            if(dataF[i].browseContrast < 0){
                                strF_app += "<td>" + dataF[i].browsePeopleCount + "(<span class='red'>"+ dataF[i].browseContrast + "%</span>)</td>";
                            }else if(dataF[i].browseContrast == 0){
                                strF_app += "<td>" + dataF[i].browsePeopleCount + "</td>";
                            }else{
                                 strF_app += "<td>" + dataF[i].browsePeopleCount + "(<span>"+ dataF[i].browseContrast + "%</span>)</td>";
                            }

                            if(dataF[i].orderPeopleContrast < 0){
                                strF_app += "<td>" + dataF[i].orderPeopleCount + "(<span class='red'>"+ dataF[i].orderPeopleContrast + "%</span>)</td>";
                            }else if(dataF[i].orderPeopleContrast == 0){
                                strF_app += "<td>" + dataF[i].orderPeopleCount + "</td>";
                            }else{
                                strF_app += "<td>" + dataF[i].orderPeopleCount + "(<span>"+ dataF[i].orderPeopleContrast + "%</span>)</td>";
                            }

                            if(dataF[i].orderMoneyContrast < 0){
                                strF_app += "<td>¥" + dataF[i].orderTotalMoney + "(<span class='red'>"+ dataF[i].orderMoneyContrast + "%</span>)</td>";
                            }else if(dataF[i].orderMoneyContrast == 0){
                                strF_app += "<td>¥" + dataF[i].orderTotalMoney + "</td>";
                            }else{
                                strF_app += "<td>¥" + dataF[i].orderTotalMoney + "(<span>"+ dataF[i].orderMoneyContrast + "%</span>)</td>";
                            }

                            if(dataF[i].payPeopleContrast < 0){
                                strF_app += "<td>" + dataF[i].payPeopleCount + "(<span class='red'>"+ dataF[i].payPeopleContrast + "%</span>)</td>";
                            }else if(dataF[i].payPeopleContrast == 0){
                                strF_app += "<td>" + dataF[i].payPeopleCount + "</td>";
                            }else{
                                strF_app += "<td>" + dataF[i].payPeopleCount + "(<span>"+ dataF[i].payPeopleContrast + "%</span>)</td>";
                            }

                            if(dataF[i].cntContrast < 0){
                                strF_app += "<td>" + dataF[i].cnt + "(<span class='red'>"+ dataF[i].cntContrast + "%</span>)</td>";
                            }else if(dataF[i].cntContrast == 0){
                                strF_app += "<td>" + dataF[i].cnt + "</td>";
                            }else{
                                strF_app += "<td>" + dataF[i].cnt + "(<span>"+ dataF[i].cntContrast + "%</span>)</td>";
                            }

                            if(dataF[i].totalAmtContrast < 0){
                                strF_app += "<td>¥" + dataF[i].totalAmt + "(<span class='red'>"+ dataF[i].totalAmtContrast + "%</span>)</td>";
                            }else if(dataF[i].totalAmtContrast == 0){
                                strF_app += "<td>¥" + dataF[i].totalAmt + "</td>";
                            }else{
                                strF_app += "<td>¥" + dataF[i].totalAmt + "(<span>"+ dataF[i].totalAmtContrast + "%</span>)</td>";
                            }

                            if(dataF[i].salesContrast<0){
                                strF_app += "<td>" + dataF[i].productTotalSales + "(<span class='red'>"+ dataF[i].salesContrast + "%</span>)</td>";
                            }else if(dataF[i].salesContrast == 0){
                                strF_app += "<td>" + dataF[i].productTotalSales + "</td>";
                            }else{
                                strF_app += "<td>" + dataF[i].productTotalSales + "(<span>"+ dataF[i].salesContrast + "%</span>)</td>";
                            }

                            if(dataF[i].priceContrast<0){
                                strF_app += "<td>¥" + dataF[i].unitPrice + "(<span class='red'>"+ dataF[i].priceContrast + "%</span>)</td>";
                            }else if(dataF[i].priceContrast == 0){
                                strF_app += "<td>¥" + dataF[i].unitPrice + "</td>";
                            }else{
                                strF_app += "<td>¥" + dataF[i].unitPrice + "(<span>"+ dataF[i].priceContrast + "%</span>)</td>";
                            }

                            if(dataF[i].PRPContrast<0){
                                strF_app += "<td>" + dataF[i].orderPRP + "%(<span class='red'>"+ dataF[i].PRPContrast + "%</span>)</td>";
                            }else if(dataF[i].PRPContrast == 0){
                                strF_app += "<td>" + dataF[i].orderPRP + "%</td>";
                            }else{
                                strF_app += "<td>" + dataF[i].orderPRP + "%(<span>"+ dataF[i].PRPContrast + "%</span>)</td>";
                            }
                            strF_app += "<tr>";
                            $("#table_fa").append(strF_app);
                        }else if(dataF[i].channel == "微信"){
                            strF_wechat = "<tr>";
                            strF_wechat += "<td>" + dataF[i].date + "</td>";
                            if(dataF[i].browseContrast < 0){
                                strF_wechat += "<td>" + dataF[i].browsePeopleCount + "(<span class='red'>"+ dataF[i].browseContrast + "%</span>)</td>";
                            }else if(dataF[i].browseContrast == 0){
                                strF_wechat += "<td>" + dataF[i].browsePeopleCount + "</td>";
                            }else{
                                 strF_wechat += "<td>" + dataF[i].browsePeopleCount + "(<span>"+ dataF[i].browseContrast + "%</span>)</td>";
                            }

                            if(dataF[i].orderPeopleContrast < 0){
                                strF_wechat += "<td>" + dataF[i].orderPeopleCount + "(<span class='red'>"+ dataF[i].orderPeopleContrast + "%</span>)</td>";
                            }else if(dataF[i].orderPeopleContrast == 0){
                                strF_wechat += "<td>" + dataF[i].orderPeopleCount + "</td>";
                            }else{
                                strF_wechat += "<td>" + dataF[i].orderPeopleCount + "(<span>"+ dataF[i].orderPeopleContrast + "%</span>)</td>";
                            }

                            if(dataF[i].orderMoneyContrast < 0){
                                strF_wechat += "<td>¥" + dataF[i].orderTotalMoney + "(<span class='red'>"+ dataF[i].orderMoneyContrast + "%</span>)</td>";
                            }else if(dataF[i].orderMoneyContrast == 0){
                                strF_wechat += "<td>¥" + dataF[i].orderTotalMoney + "</td>";
                            }else{
                                strF_wechat += "<td>¥" + dataF[i].orderTotalMoney + "(<span>"+ dataF[i].orderMoneyContrast + "%</span>)</td>";
                            }

                            if(dataF[i].payPeopleContrast < 0){
                                strF_wechat += "<td>" + dataF[i].payPeopleCount + "(<span class='red'>"+ dataF[i].payPeopleContrast + "%</span>)</td>";
                            }else if(dataF[i].payPeopleContrast == 0){
                                strF_wechat += "<td>" + dataF[i].payPeopleCount + "</td>";
                            }else{
                                strF_wechat += "<td>" + dataF[i].payPeopleCount + "(<span>"+ dataF[i].payPeopleContrast + "%</span>)</td>";
                            }

                            if(dataF[i].cntContrast < 0){
                                strF_wechat += "<td>" + dataF[i].cnt + "(<span class='red'>"+ dataF[i].cntContrast + "%</span>)</td>";
                            }else if(dataF[i].cntContrast == 0){
                                strF_wechat += "<td>" + dataF[i].cnt + "</td>";
                            }else{
                                strF_wechat += "<td>" + dataF[i].cnt + "(<span>"+ dataF[i].cntContrast + "%</span>)</td>";
                            }

                            if(dataF[i].totalAmtContrast < 0){
                                strF_wechat += "<td>¥" + dataF[i].totalAmt + "(<span class='red'>"+ dataF[i].totalAmtContrast + "%</span>)</td>";
                            }else if(dataF[i].totalAmtContrast == 0){
                                strF_wechat += "<td>¥" + dataF[i].totalAmt + "</td>";
                            }else{
                                strF_wechat += "<td>¥" + dataF[i].totalAmt + "(<span>"+ dataF[i].totalAmtContrast + "%</span>)</td>";
                            }

                            if(dataF[i].salesContrast<0){
                                strF_wechat += "<td>" + dataF[i].productTotalSales + "(<span class='red'>"+ dataF[i].salesContrast + "%</span>)</td>";
                            }else if(dataF[i].salesContrast == 0){
                                strF_wechat += "<td>" + dataF[i].productTotalSales + "</td>";
                            }else{
                                strF_wechat += "<td>" + dataF[i].productTotalSales + "(<span>"+ dataF[i].salesContrast + "%</span>)</td>";
                            }

                            if(dataF[i].priceContrast<0){
                                strF_wechat += "<td>¥" + dataF[i].unitPrice + "(<span class='red'>"+ dataF[i].priceContrast + "%</span>)</td>";
                            }else if(dataF[i].priceContrast == 0){
                                strF_wechat += "<td>¥" + dataF[i].unitPrice + "</td>";
                            }else{
                                strF_wechat += "<td>¥" + dataF[i].unitPrice + "(<span>"+ dataF[i].priceContrast + "%</span>)</td>";
                            }

                            if(dataF[i].PRPContrast<0){
                                strF_wechat += "<td>" + dataF[i].orderPRP + "%(<span class='red'>"+ dataF[i].PRPContrast + "%</span>)</td>";
                            }else if(dataF[i].PRPContrast == 0){
                                strF_wechat += "<td>" + dataF[i].orderPRP + "%</td>";
                            }else{
                                strF_wechat += "<td>" + dataF[i].orderPRP + "%(<span>"+ dataF[i].PRPContrast + "%</span>)</td>";
                            }
                            strF_wechat += "<tr>";
                            $("#table_fb").append(strF_wechat);

                        }else if(dataF[i].channel == "淘宝"){
                            strF_taobao = "<tr>";
                            strF_taobao += "<td>" + dataF[i].date + "</td>";
                            if(dataF[i].browseContrast < 0){
                                strF_taobao += "<td>" + dataF[i].browsePeopleCount + "(<span class='red'>"+ dataF[i].browseContrast + "%</span>)</td>";
                            }else if(dataF[i].browseContrast == 0){
                                strF_taobao += "<td>" + dataF[i].browsePeopleCount + "</td>";
                            }else{
                                 strF_taobao += "<td>" + dataF[i].browsePeopleCount + "(<span>"+ dataF[i].browseContrast + "%</span>)</td>";
                            }

                            if(dataF[i].orderPeopleContrast < 0){
                                strF_taobao += "<td>" + dataF[i].orderPeopleCount + "(<span class='red'>"+ dataF[i].orderPeopleContrast + "%</span>)</td>";
                            }else if(dataF[i].orderPeopleContrast == 0){
                                strF_taobao += "<td>" + dataF[i].orderPeopleCount + "</td>";
                            }else{
                                strF_taobao += "<td>" + dataF[i].orderPeopleCount + "(<span>"+ dataF[i].orderPeopleContrast + "%</span>)</td>";
                            }

                            if(dataF[i].orderMoneyContrast < 0){
                                strF_taobao += "<td>¥" + dataF[i].orderTotalMoney + "(<span class='red'>"+ dataF[i].orderMoneyContrast + "%</span>)</td>";
                            }else if(dataF[i].orderMoneyContrast == 0){
                                strF_taobao += "<td>¥" + dataF[i].orderTotalMoney + "</td>";
                            }else{
                                strF_taobao += "<td>¥" + dataF[i].orderTotalMoney + "(<span>"+ dataF[i].orderMoneyContrast + "%</span>)</td>";
                            }

                            if(dataF[i].payPeopleContrast < 0){
                                strF_taobao += "<td>" + dataF[i].payPeopleCount + "(<span class='red'>"+ dataF[i].payPeopleContrast + "%</span>)</td>";
                            }else if(dataF[i].payPeopleContrast == 0){
                                strF_taobao += "<td>" + dataF[i].payPeopleCount + "</td>";
                            }else{
                                strF_taobao += "<td>" + dataF[i].payPeopleCount + "(<span>"+ dataF[i].payPeopleContrast + "%</span>)</td>";
                            }

                            if(dataF[i].cntContrast < 0){
                                strF_taobao += "<td>" + dataF[i].cnt + "(<span class='red'>"+ dataF[i].cntContrast + "%</span>)</td>";
                            }else if(dataF[i].cntContrast == 0){
                                strF_taobao += "<td>" + dataF[i].cnt + "</td>";
                            }else{
                                strF_taobao += "<td>" + dataF[i].cnt + "(<span>"+ dataF[i].cntContrast + "%</span>)</td>";
                            }

                            if(dataF[i].totalAmtContrast < 0){
                                strF_taobao += "<td>¥" + dataF[i].totalAmt + "(<span class='red'>"+ dataF[i].totalAmtContrast + "%</span>)</td>";
                            }else if(dataF[i].totalAmtContrast == 0){
                                strF_taobao += "<td>¥" + dataF[i].totalAmt + "</td>";
                            }else{
                                strF_taobao += "<td>¥" + dataF[i].totalAmt + "(<span>"+ dataF[i].totalAmtContrast + "%</span>)</td>";
                            }

                            if(dataF[i].salesContrast<0){
                                strF_taobao += "<td>" + dataF[i].productTotalSales + "(<span class='red'>"+ dataF[i].salesContrast + "%</span>)</td>";
                            }else if(dataF[i].salesContrast == 0){
                                strF_taobao += "<td>" + dataF[i].productTotalSales + "</td>";
                            }else{
                                strF_taobao += "<td>" + dataF[i].productTotalSales + "(<span>"+ dataF[i].salesContrast + "%</span>)</td>";
                            }

                            if(dataF[i].priceContrast<0){
                                strF_taobao += "<td>¥" + dataF[i].unitPrice + "(<span class='red'>"+ dataF[i].priceContrast + "%</span>)</td>";
                            }else if(dataF[i].priceContrast == 0){
                                strF_taobao += "<td>¥" + dataF[i].unitPrice + "</td>";
                            }else{
                                strF_taobao += "<td>¥" + dataF[i].unitPrice + "(<span>"+ dataF[i].priceContrast + "%</span>)</td>";
                            }

                            if(dataF[i].PRPContrast<0){
                                strF_taobao += "<td>" + dataF[i].orderPRP + "%(<span class='red'>"+ dataF[i].PRPContrast + "%</span>)</td>";
                            }else if(dataF[i].PRPContrast == 0){
                                strF_taobao += "<td>" + dataF[i].orderPRP + "%</td>";
                            }else{
                                strF_taobao += "<td>" + dataF[i].orderPRP + "%(<span>"+ dataF[i].PRPContrast + "%</span>)</td>";
                            }
                            strF_taobao += "<tr>";
                            $("#table_fc").append(strF_taobao);
                        }
                    }
                }
            }
